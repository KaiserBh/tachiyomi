...
class PrivacyPolicyStep : OnboardingStep {
    ...
    fun MoreInfoText(url: String, modifier: Modifier = Modifier) {
        ...
        val annotatedString = buildAnnotatedString {
            withStyle(style = SpanStyle(color = Color(0xFF0645AD), fontSize = 16.sp)) {
                append(stringResource(resource = MR.strings.learn_more))
            }
            addStringAnnotation(
                tag = "URL",
                annotation = url,
                start = 0,
                end = stringResource(resource = MR.strings.learn_more).length,
            )
        }
    ...
    fun PrivacyPolicyIntro(modifier: Modifier = Modifier) {
        Column(modifier = modifier) {
            HeadingText(stringResource(resource = MR.strings.privacy_policy))
            Spacer(20)

            BodyText(stringResource(resource = MR.strings.onboarding_privacy_intro))
            Spacer(10)

            BodyText(stringResource(resource = MR.strings.onboarding_privacy_info_overview))
            Spacer(10)

            BodyText(stringResource(resource = MR.strings.onboarding_privacy_agreement_acknowledgement))
            Spacer(20)
    ...
    fun PrivacyInformationCollectionUse(modifier: Modifier = Modifier) {
        Column(modifier = modifier) {
            HeadingText(stringResource(resource = MR.strings.onboarding_privacy_information_collection_use))
            Spacer(20)
            ...
            MoreInfoText("https://tachiyomi.org/privacy/")
        }
    }
}
